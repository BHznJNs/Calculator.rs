...
use super::composer::{array_reading, object_reading, compose};
...
pub fn resolve(
    node:  Rc<AssignmentNode>,
    scope: &mut Scope
) -> Result<Value, ()> {
    let left_hand_node =
        &node.left_hand_node;
    let right_hand_clone =
        node.right_hand_node.clone();
    let right_hand_value =
        expression::resolve(right_hand_clone.into(), scope)?;
    ...
    match left_hand_node {
        ...
        ASTNode::ArrayElementReading(sub_node) => {
            let array_clone =
                sub_node.array_node.clone();
            let array_value =
                compose::resolve(array_clone.into(), scope)?;
        ...
        ASTNode::ObjectReading(sub_node) => {
            let obj_clone =
                sub_node.obj_node.clone();
            let obj_value =
                compose::resolve(obj_clone.into(), scope)?;
